...
def get_nii_data(file_path):
    ...
    return flattened_beta_data[top_voxel_indices]

No_OF_TOP_VOXELS = 1600  # Number of top voxels to extract
NO_OF_TRIALS = 24
...
for subject in subjects:    
    ...
    subject_Y[subject] = Y
    subject_X[subject] = [get_nii_data(base_path+file_name) for file_name in X_file_names]

for turn, subject in enumerate(results_subjects):
    print(turn, subject)
    subject_name = f"sub-{subject}"
    result_sheet[subject_name] = []
    
    X_test = subject_X[subject]
    y_test = subject_Y[subject]
    X_train = []
    y_train = []
    
    for data_subject in subjects:
        if data_subject != subject:
            X_train.extend(subject_X[data_subject])
            y_train.extend(subject_Y[data_subject])
...
df.to_csv(f"{No_OF_TOP_VOXELS}_1_voxel_results.csv")    